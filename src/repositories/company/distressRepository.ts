...
export default class distressRepository {


  public static async getDistressProperties(page: number, filters: PropertyFilterType,view: string) {
    console.log(filters)
    const baseUrl = import.meta.env.VITE_PROPERTIES_APP_BASE_URL + `/properties/distress`;
  ...
  public static async getDistressProperty(id: string) {
    const url = import.meta.env.VITE_PROPERTIES_APP_BASE_URL + `/properties/distress/${id}`;
    const authToken = `Bearer ${Cookies.get('props_token')}`;
