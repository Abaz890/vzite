...
export default function CompanyAgentList() {
  ...
  const [agents, setAgents] = useState<AgentsPayload>();
  ...
  const handleDelete = async function () {
    const response = await companyAgentRepository.deleteAgent(itemId!.toString());
    if (response.success) {
      setConfirmDeleteModalOpen(!confirmDeleteModalOpen);
      toast({
        variant: "default",
...
className: "top-0 right-0 flex fixed md:max-w-[420px] md:top-4 md:right-4",
  ...
  return !isInitalLoading ? (
    <div className="">
      <CompanyActions section_title="Agents" section_name="agent"></CompanyActions>
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <div className="hidden md:block ">
            {!isInitalLoading && agents! && agents!.data.length > 0 ? (
              <Table>
                <TableHeader>
                  <TableRow>
                        ...
                        {key !== 0 ? (
                          <DropdownMenu modal={false}>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem onClick={() => handleDeleteItemClicked(agent.id)}>Delete</DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                        ) : (
                          <></>
              ...
              <Table>
                ...
                <TableBody>
                  {agents!.data.length > 0
                    ? agents!.data.map((agent: any, item_key) => (
        ...
        <CardFooter className="flex-col items-start">
          {!loading && agents ? (
            <div className="text-xs text-muted-foreground">
              ...
              Showing{" "}
              <strong>
                {agents!.meta.from} - {agents!.meta.to}
              </strong>{" "}
              of <strong>{agents!.meta.total}</strong> agents{" "}
            </div>
          ) : (
            <></>
          )}
          <Pagination className="my-2 justify-start">
            <PaginationContent className="flex flex-wrap justify-center gap-2">
              {!loading && agents ? (
                agents!.meta.links.map((link, index) => {
