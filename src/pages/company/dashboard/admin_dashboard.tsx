...
export default function CompanyAdminDashboard() {
  ...
  const [managementModule, setManagementModule] = useState<{
    permalink: string;
    default_view: string;
  }>();
  ...
  const fetchModuleImprotPerp = async (file: File) => {
    try {
      ...
      const response = await companyModuleRepository.getModuleImportPerp(managementModule!.permalink, file);
      if (response.success) {
        ...
        setFormData((prevState: Record<string, unknown>) => ({
          ...prevState,
          [`${managementModule!.permalink}__status__value`]: "pending",
          [`${managementModule!.permalink}__status__source`]: "default_value",
  ...
  const handleImport = async () => {
    ...
    const response = await companyModuleRepository.getModuleImportExec(managementModule!.permalink, reversed, file!);
    if (response.success) {
      toast({
        variant: "default",
        duration: 800,
className: "top-0 right-0 flex fixed md:max-w-[420px] md:top-4 md:right-4",
      ...
      fetchImportTasks(managementModule!.permalink);
    }
...
//     const baseModule = activeModules.value.find((module: { name: string }) => module.name === "management");
...
//   };
  ...
  const handleDeleteImportTask = async () => {
    if (delete_import_task_id) {
      ...
      const response = await companyModuleRepository.deleteModuleImportTask(managementModule!.permalink, delete_import_task_id).finally(() => {
        setDeleteImportTaskLoading(false);
      });
      if (response.success) {
        fetchDeleteImportTasks(managementModule!.permalink);
        const updatedImportTasks = await companyModuleRepository.getModuleImportTasks(managementModule!.permalink);
...
className: "top-0 right-0 flex fixed md:max-w-[420px] md:top-4 md:right-4",
  ...
  useEffect(() => {
    if (!globalStateLoading) {
      ...
      setManagementModule(baseModule);
