...
import companyDistressRepository from "@/repositories/company/companyDistressRepository";
import adminDistressPropertyRepository from "@/repositories/admin/adminDistressPropertyRepository";

export default function AdministratorDistressPropertyList() {
  ...
  const fetchProperties = async () => {
    ...
    const response = await companyDistressRepository.getProperties(page, filters, 'list');
    if (response.success) {
  ...
  const handleAddProperty = async () => {
    ...
    if (response.success) {
      navigate(`/administrator/distress/${response.data.reference_id}`);
    }
  };

  const handleUpdatPropertyStatus = async (id: string) => {

    const response = await adminDistressPropertyRepository.updatePropertyStatus(id);
    if (response.success) {
  ...
  const handleDeleteItem = async (id: any) => {
    ...
    const response = await adminDistressPropertyRepository.deleteProperty(id);
    if (response.success) {
      if (page > 1) {
        setPage(1);
      }
    ...
    <>
      ...
      >
        ...
        <PropertyLayout>
          <Card>
            <CardHeader>
              <CardTitle>Distress Properties</CardTitle>
              <CardDescription>
                Manage Distress Properties.
              </CardDescription>
            </CardHeader>
            <CardContent>
                          ...
                          {properties!.data.map((property: any, key: Key) => (
                            <TableRow key={key}>
                              <TableCell> {property.media.length > 0 ?
                                <LazyImage src={property.media[0].url} alt={``} width={80} className={` aspect-[16/9] rounded-t-lg`} /> : <img src="" alt="" />} </TableCell>
                              ...
                              <TableCell className="font-medium">
                                {
                                  Number(property.meta.roi) > 0 ? <Badge
                                    className={cn(
                                      "bg-foreground text-background text-xs font-medium",
                                      property.meta.is_roi_approximate > 0 ? "bg-amber-500/90 text-amber-950" : '',
                                    )}
                                  >
                                    {
                                      property.meta.is_roi_approximate > 0 ? <div >{"~"}</div> : null
                                    }

                                    {Number(property.meta.roi).toFixed(2)} %
                                  </Badge>
                              ...
                              <TableCell>
                                <DropdownMenu modal={false}>
                                  ...
                                  <DropdownMenuContent align="end">
                                    ...
                                    <DropdownMenuItem onClick={() => navigate(`/administrator/distress_properties/${property.reference_id}`)}  >Edit</DropdownMenuItem>
