...
import BasicInfoStep from "@/components/off_plan_property_add_form/basic-info-step";
import LocationStep from "@/components/off_plan_property_add_form/location-step";
import MediaStep from "@/components/off_plan_property_add_form/media-step";
import AmenitiesStep from "@/components/off_plan_property_add_form/amenities-step";
import SeoStep from "@/components/off_plan_property_add_form/seo-step";
import PermitsAndLicensesStep from "@/components/off_plan_property_add_form/permits-and-licenses-step";
...
import UnitsStep from "@/components/off_plan_property_add_form/units-step";
import { useGlobalState } from "@/providers/globalContext";
import AttachmentsStep from "@/components/off_plan_property_add_form/attachments-step";
import PaymentPlanStep from "@/components/off_plan_property_add_form/payment-plan-step";

export default function AdministratorOffplanPropertyEdit() {
  ...
  const fetchProperty = async () => {
    if (params.property_id) {
      ...
      const response = await adminOffPlanPropertyRepository.getProperty(params.property_id, 'admin');
      if (response.success) {
        ...
        //attachment
        setPropertyAttachments(property.attachments);
      }
    }
  ...
  const handleUpdateProperty = async (data: any) => {
    if (!isFetchingRef.current) {
      ...
      const response = await adminOffPlanPropertyRepository.updateProperty(params.property_id!, transformedData);
      if (response.success) {
        toast({
          variant: "default",
          duration: 800,
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full min-h-[70dvh]">
              ...
              <div className="relative">
                <div className={activeTab === "basic-info" ? "block" : "hidden"}>
                  <BasicInfoStep form={form} property={propertyPayload} />
                </div>
                ...
                <div className={activeTab === "payment_plans" ? "block" : "hidden"}>
                  <PaymentPlanStep form={form} />
                </div>

                <div className={activeTab === "attachments" ? "block" : "hidden"}>
                  <AttachmentsStep form={form} media={propertyAttachments} />
                </div>
                ...
                {activeTab === "seo" ? (
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      if (success) {
                        navigate("/administrator/offplan_properties/list");
                      }
                    }}
                  >
                    {isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
